 ...
 * @categories Configuration_Variables
 */
//_str def_build_warnings_re="warning|CID";
_str def_build_warnings_re="CID";
   ...
   if ( load_rc ) {
      ...
      if (!load_rc) {
         ...
         if (_top_process_mark=="") {
            ...
            _select_type(_top_process_mark,'A',0 /* don't adjust column for backward compatibility*/ );
            top();
   ...
   for (;;) {
      ...
      if ( filename != "" ) {
         ...
         if (!status) {
            ...
            // drop pic here
            if ( setErrorMarkers ) {
               markerHandle = _LineMarkerAdd(p_window_id, p_line, false, 0, error_bmp,
...
_command int next_error(_str resetMessages="",
                        ...
                        _str lookingBackwards="") name_info(','VSARG2_EDITORCTL|VSARG2_READ_ONLY|VSARG2_REQUIRES_MDI)
{
   if (isEclipsePlugin()) {
 ...
 *
 */
_command int prev_error() name_info(','VSARG2_EDITORCTL|VSARG2_READ_ONLY|VSARG2_REQUIRES_MDI)
{
   if (isEclipsePlugin()) {
...
_command cursor_error2() name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL|VSARG2_REQUIRES_MDI)
{
   ...
   if (!status) {
      ...
      push := !((p_buf_flags & VSBUFFLAG_HIDDEN) || p_buf_name == ".process");
   ...
   if (!status) {
      ...
      if (!_process_info('b') || _top_process_mark=="" || _select_type(_top_process_mark)=="") {
         ...
         _str orig_top_process_mark=_top_process_mark;
         ...
         _top_process_mark=new_top_mark;
         restore_pos(p3);_begin_line();

         status=call_index(_error_search);

         _top_process_mark=orig_top_process_mark;
...
static void cacheExpressionDetails(ERRORRE_INFO &info, COLLECTION_ITEM (&collection)[])
{
   ...
   if (enabled) {
      collection[collection._length()].regex = info.m_re;
      collection[collection._length()-1].macro = info.m_macro;
   }
}
